...
//#define UNITY_2018_1_OR_NEWER // Remove this later. I should have to add this, but Unity/VisualStudio is being stupid.
...
namespace VR3D
{
    public class Stereoscopic3DScreeenshot : MonoBehaviour
    {
        ...
        public enum ScreenShotImageFormat : int { Side_By_Side, Top_Bottom, TwoImages, HorizontalInterlaced, VerticalInterlaced, Checkerboard, Anaglyph, _180, _360 };
#else
        ...
        public void GetScreenshot(Texture2D tex, Texture2D tex2 = null)
        {
            switch (imageFormat)
            {
                ...
                case ScreenShotImageFormat.Anaglyph:
                    ...
                    break;
#if UNITY_2018_1_OR_NEWER
                case ScreenShotImageFormat._180:
                    GetPanoTexture(tex);
                    break;
                case ScreenShotImageFormat._360:
                    GetPanoTexture(tex);
                    break;
#endif
                default:
        ...
        public void GetPanoTexture(Texture2D texture)
        {
            int cubeMapSize = Mathf.NextPowerOfTwo(resolutionWidth >= resolutionHeight ? resolutionWidth : resolutionHeight);
            int renderTextureSize = (imageFormat == ScreenShotImageFormat._180 ? cubeMapSize * 2 : cubeMapSize);

            texture.Resize(cubeMapSize, cubeMapSize * (imageFormat == ScreenShotImageFormat._180 ? 4 : 2));

            // Create and get a temporary camera.
            GameObject cameraGO = new GameObject("Pano Camera");
            Camera camera = cameraGO.AddComponent<Camera>();

            // Our temp camera needs to match the same position/rotation as the this object.
            cameraGO.transform.parent = transform;
            cameraGO.transform.localPosition = Vector3.zero;
            cameraGO.transform.localRotation = Quaternion.identity;

            RenderTexture leftCubemap = new RenderTexture(cubeMapSize, cubeMapSize, 24);
            RenderTexture rightCubemap = new RenderTexture(cubeMapSize, cubeMapSize, 24);
            RenderTexture leftTexture = new RenderTexture(renderTextureSize, renderTextureSize, 24);
            RenderTexture rightTexture = new RenderTexture(renderTextureSize, renderTextureSize, 24);

            leftCubemap.dimension = UnityEngine.Rendering.TextureDimension.Cube;
            rightCubemap.dimension = UnityEngine.Rendering.TextureDimension.Cube;

            // Position the camera aligned with this object.            
            camera.stereoSeparation = (seperation / 1000);
            
            camera.RenderToCubemap(leftCubemap, 63, Camera.MonoOrStereoscopicEye.Left);
            camera.RenderToCubemap(rightCubemap, 63, Camera.MonoOrStereoscopicEye.Right);

            // We use Mono to get a full sized texture. We can halve it later if we want.
            leftCubemap.ConvertToEquirect(leftTexture, Camera.MonoOrStereoscopicEye.Mono);
            rightCubemap.ConvertToEquirect(rightTexture, Camera.MonoOrStereoscopicEye.Mono);

            //            
            Rect leftTexureRect = new Rect();
            leftTexureRect.x = (imageFormat == ScreenShotImageFormat._180 ? leftTexture.width / 4 : 0);
            leftTexureRect.y = 0;
            leftTexureRect.width = (imageFormat == ScreenShotImageFormat._180 ? leftTexture.width / 2 : leftTexture.width);
            leftTexureRect.height = leftTexture.height;

            Rect rightTexureRect = new Rect();
            rightTexureRect.x = (imageFormat == ScreenShotImageFormat._180 ? rightTexture.width / 4 : 0);
            rightTexureRect.y = 0;
            rightTexureRect.width = (imageFormat == ScreenShotImageFormat._180 ? rightTexture.width / 2 : rightTexture.width);
            rightTexureRect.height = rightTexture.height;

            RenderTexture.active = leftTexture;
            texture.ReadPixels(leftTexureRect, 0, rightTexture.height);
            RenderTexture.active = rightTexture;
            texture.ReadPixels(rightTexureRect, 0, 0);

            texture.Apply();

            //
            ResizeTexture(texture, resolutionWidth, imageSize == ScreenShotImageSize.Half ? resolutionHeight : resolutionHeight * 2);
