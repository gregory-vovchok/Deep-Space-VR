...
namespace VR3D
{
    ...
    /// <summary>
    /// Which type of method to use to achieve stereoscopic 3d rendering.
    /// </summary>
    public enum RenderingMethod : int { Layered, Shader };
    ...
    /// 
    /// </summary>
    public enum StereoRenderingPath : int { MultiPass, SinglePass, Instancing };
    ...
    {
        private static VR3DSettings _settings;
        public static RenderingMethod RenderingMethod
        {
            get
            {
                if (_settings == null)
                {
                    VR3DSettings[] objs = Resources.FindObjectsOfTypeAll<VR3DSettings>();

                    if (objs.Length > 0)
                        _settings = objs[0];
                    else
                    {
                        Debug.LogWarning("[VR3DMediaViewer] Unable to find \"VR3DSettings\". Did you delete it?");
                        return 0;
                    }

                }
#if UNITY_2018_2_OR_NEWER                
                return _settings.RenderingMethod;
#else
                return ((int)_PlayerSettings.stereoRenderingPath >= 1 && _PlayerSettings.virtualRealitySupported ? RenderingMethod.Shader : _settings.RenderingMethod);
#endif
            }
        }        
        ...
        //[HideInInspector]
        //public RenderingMethod multiPassSelection = RenderingMethod.Layered;
        ...
        public ImageFormat ImageFormat
        {
            get 
            {
                if (m_displayCanvas == null) m_displayCanvas = new DisplayCanvas(gameObject, RenderingMethod == RenderingMethod.Layered);
        ...
        {   
            get 
            {
                ...
                if (currentImage != null)
                    ...
                    return false; // If theres no S3D image, we don't swap anything.
            }
            set
            {
                if (m_displayCanvas == null) m_displayCanvas = new DisplayCanvas(gameObject, RenderingMethod == RenderingMethod.Layered);

                if (currentImage == null || value == currentImage.swapLeftRight) return;
        ...
        [HideInInspector]
        [SerializeField]
        private Shader m_singlePassShader;
        ...
        void Awake () 
        {
            ...
            m_displayCanvas = new DisplayCanvas(gameObject, RenderingMethod == RenderingMethod.Layered, targetTextureMaps);
            ...
            if (defaultImage != null) DisplayImageInCanvas(defaultImage, RenderingMethod == RenderingMethod.Layered);
        }
        ...
        private void DisplayImageInCanvas(Stereoscopic3DImage s3DImage, bool layered)
        {
            if (m_displayCanvas == null) m_displayCanvas = new DisplayCanvas(gameObject, layered);
        ...
        {
            // This get set here while we have a reference to the texture, so that m_displayCanvas.CanvasFormat can set the material properly.
            if (currentImage.sourceTexture != null)
                m_displayCanvas._180Clamp = (currentImage.sourceTexture.wrapMode == TextureWrapMode.Clamp);
#if UNITY_5_6_PLUS
            else if (currentImage.videoClip != null || currentImage.videoURL != string.Empty)
            {
                VideoPlayer videoPlayer = gameObject.GetComponent<VideoPlayer>();
                if (videoPlayer != null)
                    m_displayCanvas._180Clamp = (videoPlayer.texture.wrapMode == TextureWrapMode.Clamp);
            }
#endif
            ...
            switch (currentImage.imageFormat)
            {
                ...
                default:
                        ...
                        texture2 != null)
                    {
                        ...
                        m_displayCanvas.RightTexture = (currentImage.swapLeftRight ? texture : (texture2 != null && currentImage.imageFormat == ImageFormat.TwoImages ? texture2 : texture));
                    }
                    else
                    {
...
#region Public Functions for Runtime Use
        ...
        {
            DisplayImageInCanvas(newS3DImage, RenderingMethod == RenderingMethod.Layered);
        }
        ...
        public class DisplayCanvas
        {
                ...
                set
                {
                    ...
                    {   
                        if (isLayered)
                            ...
                            //m_leftImageMaterial.SetVector("_LeftMapping", new Vector4(value.x, value.y));
                            m_leftImageMaterial.mainTextureScale = value;
                ...
                set
                {
                        ...
                        (UsingRawImage && !isLayered))
                    {
                        if (isLayered)
                            ...
                            m_leftImageMaterial.SetTextureScale("_RightTex", value);
            ...
            public Vector2 LeftMainTextureOffset
            {
                ...
                set
                {
                        ...
                        (UsingRawImage && !isLayered))
                    {
                        ...
                        {
                            //Vector4 v = m_leftImageMaterial.GetVector("_LeftMapping");
                            //m_leftImageMaterial.SetVector("_LeftMapping", new Vector4(v.x, v.y, value.x, value.y));
                            m_leftImageMaterial.mainTextureOffset = value;
                        }
            ...
            public Vector2 RightMainTextureOffset
            {
                ...
                set
                {
                        ...
                        (UsingRawImage && !isLayered))
                    {
                        ...
                        {
                            //Vector4 v = m_leftImageMaterial.GetVector("_RightMapping");
                            //m_leftImageMaterial.SetVector("_RightMapping", new Vector4(v.x, v.y, value.x, value.y));
                            m_leftImageMaterial.SetTextureOffset("_RightTex", value);
                        }
            ...
            public Texture RightTexture
            {
                get
                {
                    if (UsingRawImage && isLayered)
                        ...
                        return (isLayered ? m_rightImageMaterial.mainTexture : m_leftImageMaterial.mainTexture);
                }
                set
                {
                    if (UsingRawImage && isLayered)
                    ...
                    else
                    {
                        ...
                        {
                            m_leftImageMaterial.SetTexture("_RightTex", value);

                            if (UsingRawImage) m_leftRawImage.SetAllDirty();
                        }
                    }
                }
            }
                ...
                set
                {
                    ...
                    if (UsingRawImage)
                    {
                                ...
                                m_canvasFormat == CanvasFormat._180))
                        {
                            ...
                            m_leftImageMaterial.SetFloat("_OneEightyClamp", (m_180Clamp ? 1 : 0));
                            m_rightImageMaterial.SetFloat("_OneEightyClamp", (m_180Clamp ? 1 : 0));

                            if (!isLayered) m_leftImageMaterial.SetFloat("_SinglePass", 1);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (!isLayered)
                            {
                                m_leftImageMaterial.shader = Shader.Find("Hidden/VR3D/SinglePass3DUnlit");
                            }
                            else
                            {
            ...
            private bool m_180Clamp = false;
            public bool _180Clamp
            {
                get { return m_180Clamp; }
                set
                {
                    m_180Clamp = value;
                }
            ...
            public DisplayCanvas(GameObject gameObject, bool layered, string[] targetTextureMaps = null)
            {
            ...
            private void SetupCanvas()
            {
                ...
                if (m_rendererComponent != null)
                {
                    ...
                    {
                        m_leftImageMaterial = m_rendererComponent.material;
                        m_rightImageMaterial = m_rendererComponent.material;
                    }
                }
                else if (m_rawImageComponent != null)
                {
                    ...
                    {
                        // We use the settings in the material as our default.
                        m_rawImageComponent.material = new Material(Shader.Find("Hidden/VR3D/SinglePass3DUnlit"));

                        m_defaultMaterial = m_rawImageComponent.material;

                        m_defaultMainTextureScale = m_defaultMaterial.mainTextureScale;
                        m_defaultMainTextureOffset = m_defaultMaterial.mainTextureOffset;

                        m_leftImageMaterial = m_defaultMaterial;
                        m_rightImageMaterial = m_defaultMaterial;

                        m_leftRawImage = m_rawImageComponent;
                        m_rightRawImage = m_rawImageComponent;
                    }
                }
                else
                {
