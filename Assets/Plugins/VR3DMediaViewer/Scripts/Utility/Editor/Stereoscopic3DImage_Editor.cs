...
public class Stereoscopic3DImage_Editor : Editor
{
    ...
    private void ImageSettingsWarnings(Texture texture, string label)
    {
        ...
        if (label != "Right Texture")
        {
            ...
            EditorGUILayout.LabelField("Recommended Texture Import Settings:", EditorStyles.boldLabel);

            if (GUILayout.Button("Apply Recommendations", GUILayout.Width(150)))
            {
        ...
        if (textureImporter.npotScale != TextureImporterNPOTScale.None && !isPowerOf2(texture))
            EditorGUILayout.HelpBox("Non Power of 2 = \t\t" + textureImporter.npotScale.ToString() + " (Recommended: None)", (isInterlaced ? MessageType.Warning : MessageType.Info));

        if (textureImporter.mipmapEnabled)
            EditorGUILayout.HelpBox("Gerate Mip Maps = \ttrue" + " (Recommended: false)", (isInterlaced ? MessageType.Warning : MessageType.Info));

        if (textureImporter.filterMode != FilterMode.Point)
            EditorGUILayout.HelpBox("Filter Mode = \t\t" + (textureImporter.filterMode == FilterMode.Bilinear ? FilterMode.Bilinear : FilterMode.Trilinear).ToString() + " (Recommended: Point)", (isInterlaced ? MessageType.Warning : MessageType.Info));
        ...
        if (textureImporter.textureCompression != TextureImporterCompression.Uncompressed)
        {
            ...
            EditorGUILayout.HelpBox("Compression = \t\t" + compressionText + " (Recommended: None)", (isInterlaced ? MessageType.Warning : MessageType.Info));
        }
