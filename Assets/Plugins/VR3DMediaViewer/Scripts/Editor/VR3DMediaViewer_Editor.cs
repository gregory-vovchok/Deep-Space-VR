...
public class VR3DMediaViewer_Editor : Editor 
{
    ...
    private SerializedProperty defaultImageProperty;
    private SerializedProperty targetTextureMapsProperty;

    void OnEnable()
    {
        theScript = (VR3DMediaViewer)target;

        defaultImageProperty = serializedObject.FindProperty("defaultImage");
        targetTextureMapsProperty = serializedObject.FindProperty("targetTextureMaps");
    }
	...
	public override void OnInspectorGUI()
	{
        ...
        if (
            ...
            PlayerSettings.stereoRenderingPath == UnityEditor.StereoRenderingPath.MultiPass &&
#endif
            VR3DMediaViewer.RenderingMethod == RenderingMethod.Layered)
            CheckCameras();
        ...
        if (EditorGUILayout.PropertyField(targetTextureMapsProperty))
        {
            EditorGUI.indentLevel++;
            
            EditorGUI.BeginChangeCheck();

            int size = EditorGUILayout.DelayedIntField("Size", targetTextureMapsProperty.arraySize);

            if (EditorGUI.EndChangeCheck())
                targetTextureMapsProperty.arraySize = size;

            for (int i = 0; i < size; i++)
            {
                SerializedProperty elementProperty = targetTextureMapsProperty.GetArrayElementAtIndex(i);
        ...
        if (Application.isPlaying)
        {
            ...
            EditorGUILayout.ObjectField(new GUIContent("Current Image"), theScript.currentImage, typeof(Stereoscopic3DImage), true);
    ...
    void CheckCameras()
    {
        ...
        {
            // Lets not show the warning for our own example camera, as not to confuse people.
            if (camera.transform.parent != null && 
                (camera.transform.parent.name == "Example Cross-eyed Camera Rig" ||
                camera.transform.parent.GetComponent<DemoCameraManager>()))
            {
