		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					int currentYPixel = abs(i.uv.y * _MainTex_TexelSize.w);
					...
					if (i.uv.y >= 0.5)
					{
						int currentOffsetYPixel = abs(currentYPixel - (_MainTex_TexelSize.w / 2));
					...
					{
						int currentOffsetYPixel = abs(currentYPixel + 1);
			...
			Pass
			{
				...
				{
					// We get the vertical index of the pixel that should be displayed at this text cord.					
					int currentXPixel = abs(i.uv.x * _MainTex_TexelSize.z);
					...
					if (i.uv.x >= 0.5)
					{
						int currentOffsetXPixel = abs(currentXPixel - (_MainTex_TexelSize.z / 2));
					...
					{
						int currentOffsetXPixel = abs(currentXPixel + 1);
			...
			Pass
			{
				...
				{
					// We get the vertical index of the pixel that should be displayed at this text cord.
					float currentXPixel = abs(i.uv.x * _MainTex_TexelSize.z);
				float currentYPixel = abs(i.uv.y * _MainTex_TexelSize.w);
					...
					if (i.uv.x >= 0.5)
					{
						if (round(currentYPixel % 2) == 0)
						{
							int currentOffsetXPixel = abs(currentXPixel - (_MainTex_TexelSize.z / 2)) + 1;
					...
					else
					{
						if (round(currentYPixel % 2) == 0)
						{
							int currentOffsetXPixel = currentXPixel;
						...
						{
							int currentOffsetXPixel = currentXPixel + 1;
